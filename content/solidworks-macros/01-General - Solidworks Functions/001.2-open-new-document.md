...
url: /solidworks-macros/open-new-document/
weight: 2
---
...
{{< youtube id="SXrdQ0vrTyI" >}}

To do this, we first need to create a new empty macro. If you don't know how to create an empty macro; then please go to this 🚀 **{{< target-blank "VBA in Solidworks" "/Solidworks-macro-guide/" >}}** post.
...
You can see more about `ModelDoc2` in this help 🚀 **{{< target-blank "link" "http://help.solidworks.com/2025/English/api/sldworksapi/SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IModelDoc2.html?verRedirect=1" >}}**.
...
{{< callout context="note" title="Note" icon="outline/info-circle" >}}
In programming vocabulary, we need to pass the parameter to this function so that this function can work.
{{< /callout >}}
...
{{< callout context="note" title="Note" icon="outline/info-circle" >}}
Please note that there are lots of values inside this enum list. You can see these values from this 🚀 **{{< target-blank "link" "http://help.solidworks.com/2025/English/api/swconst/SOLIDWORKS.Interop.swconst~SOLIDWORKS.Interop.swconst.swUserPreferenceStringValue_e.html" >}}**.
{{< /callout >}}
...
| ---------------- | ------------------------------------------------------------------------------- |
| **TemplateName** | This can be a full path of the template, which we use to create a new document. |
...
If this method fails to create a new document, in that case this returns `NULL` value. We can use this `NULL` value to check if the operation is successful or not.
